...
RUN npm install -g deno \
  && deno --version
# The following is required for Astro `Last updated` timestamp to sucessfully access `.git` on Linux.
# It seems Astro just silently fails if any error occurs and keeps the build running.
