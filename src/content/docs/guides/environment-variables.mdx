...
1. Create the following 4 files:
...
1. Why don't commands `make clean` and `make envfile` fail when file `.env` is
   not present?
1. What would be the main reason to use
   `ENVFILE=$(ENVFILE) $(COMPOSE_RUN_ALPINE)` in targets `envfile` and `clean`
